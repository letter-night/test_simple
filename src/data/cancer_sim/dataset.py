...
class PretrainCancerDataset(SyntheticCancerDataset):
    ...
    def process_data(self, scaling_params):
        ...
        input_means = mean[['cancer_volume', 'patient_types', 'radio_application']].values.flatten()  # .astype(np.float32).flatten()
        input_stds = std[['cancer_volume', 'patient_types', 'radio_application']].values.flatten()  # .astype(np.float32).flatten()
