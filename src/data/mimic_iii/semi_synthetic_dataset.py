...
class MIMIC3SyntheticDataset(MIMIC3RealDataset):
    ...
    def __init__(self,
        ...
        Args:
            ...
            obs_mask: DataFrame with mask of observed covariates
        """
        ...
        if mode == 'factual':
            ...
            self.obs_mask = self.obs_mask.unstack(fill_value=np.nan, level=0).stack(dropna=False).swaplevel(0, 1).sort_index()
            ...
            num_unique_traj = self.all_vitals.index.get_level_values('traj').nunique()

            df_list = []

            for i in range(num_unique_traj):
                obs_mask_copy = self.obs_mask.copy()
                # Add the stacking level as a new index level
                obs_mask_copy['traj'] = i
                # Set it as the outermost index and add it to the list
                obs_mask_copy.set_index('traj', append=True, inplace=True)
                df_list.append(obs_mask_copy)

            # Concatenate all dataframes
            self.obs_mask = pd.concat(df_list)
            self.obs_mask = self.obs_mask.reorder_levels(['subject_id', 'traj', 'hours_in'])
            self.obs_mask = self.obs_mask.unstack(fill_value=np.nan, level=0).stack(dropna=False).swaplevel(0,
                                                                                                            1).sort_index()
        ...
        obs_mask = self.obs_mask.fillna(0.0).values.reshape((-1, max(user_sizes), len(vital_cols)))
        obs_mask[:, 0:2, :] = 1
        vitals[obs_mask == 0] = 'nan'
        # Apply observation mask to synthetic outcome. Observation probability p=0.15
        outcomes_mask = np.random.uniform(0, 1, size=outcomes_unscaled.shape)>0.15
        outcomes_mask[:, 0:2, :] = False
        outcomes_unscaled[outcomes_mask] = 'nan'
...
class MIMIC3SyntheticDatasetCollection(SyntheticDatasetCollection):
    ...
    def __init__(self,
        ...
        self.seed = seed
        np.random.seed(seed)
        all_vitals, static_features, obs_mask = load_mimic3_data_raw(ROOT_PATH + '/' + path, min_seq_length=min_seq_length,
        ...
        obs_mask, obs_mask_test = obs_mask.loc[static_features.index], obs_mask.loc[static_features_test.index]
        if split['val'] > 0.0:
            ...
            obs_mask_train, obs_mask_val = obs_mask.loc[static_features_train.index], obs_mask.loc[static_features_val.index]
        else:
            static_features_train = static_features
            all_vitals_train = all_vitals
            obs_mask_train = obs_mask

        self.train_f = \
            MIMIC3SyntheticDataset(all_vitals_train, static_features_train, synth_outcomes_list, synth_treatments_list,
                                   treatment_outcomes_influence, 'train', obs_mask=obs_mask_train)
        ...
        if split['val'] > 0.0:
            self.val_f = MIMIC3SyntheticDataset(all_vitals_val, static_features_val, synth_outcomes_list, synth_treatments_list,
                                                treatment_outcomes_influence, 'val', obs_mask=obs_mask_val)
        ...
        self.test_cf_one_step = \
            MIMIC3SyntheticDataset(all_vitals_test, static_features_test, synth_outcomes_list, synth_treatments_list,
                                   ...
                                   treatments_seq=None, obs_mask=obs_mask_test)
